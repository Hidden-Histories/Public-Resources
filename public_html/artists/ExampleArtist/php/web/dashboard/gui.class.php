 ...
 * includes for the google map key
...
HTML;
...

    {
        ...
        foreach ($pois as $poi) {
            ...
            if ($poi->dimension > 1) {
                ...
                $result .= sprintf("<input type=\"hidden\" name=\"relativeLocation\" value=\"%s\"> \n", $poi->object->relativeLocation);
            ...
            foreach ($poi->animations as $event => $animations) {
                ...
                foreach ($animations as $animation) {
                    ...
                    $result .= sprintf("%s\n", self::createHiddenAnimationSubtable($animationIndex, $event, $animation));
...

    {
        ...
        $result .= sprintf("<tr><td>Relative altitude</td><td><input type=\"text\" name=\"relativeAlt\" value=\"%s\" size=\"5\"></td></tr>\n", $poi->relativeAlt);
            ...
            $result .= sprintf("<tr><td>Relative location (x,y,z)</td><td><input type=\"text\" name=\"relativeLocation\" value=\"%s\"></td></tr>\n", $poi->object->relativeLocation);
